...
var optWrap = $(".opt");
var maxResult = optWrap.find(".max1").val();
var maxResult2 = optWrap.find(".max2").val();
var $reslutBtn = optWrap.find(".btn_result");



$(document).on("click", ".btn_result", function(){
    $(this).closest(".gugudan").find(".content_title").addClass("on");
    var c = counter().checking() += maxResult;
    opt($(this), c, m, n);
})



function opt(el, counter) {
...
$(document).on("click", ".prev_btn1", function(){
    perv($(this), counter01);  //9단, 9단계 까지 노출
});
$(document).on("click", ".next_btn1", function(){
    next($(this), counter01);  //9단, 9단계 까지 노출
    if($(this).text("시작")){
        $(this).text("다음");
    }
});
